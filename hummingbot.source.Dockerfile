...
COPY hummingbot/setup/environment.yml /tmp/environment.yml
RUN conda env create -f /tmp/environment.yml && \
    conda clean -afy && \
    rm /tmp/environment.yml

# Copy remaining files
COPY hummingbot/bin/ bin/
COPY hummingbot/hummingbot/ hummingbot/
COPY hummingbot/scripts/ scripts/
COPY hummingbot/controllers/ controllers/
COPY hummingbot/scripts/ scripts-copy/
COPY hummingbot/setup.py .
COPY hummingbot/LICENSE .
COPY hummingbot/README.md .

# activate hummingbot env when entering the CT
SHELL [ "/bin/bash", "-lc" ]
RUN echo "conda activate hummingbot" >> ~/.bashrc

COPY hummingbot/setup/pip_packages.txt /tmp/pip_packages.txt
...
CMD /bin/bash -c "source /opt/conda/bin/activate hummingbot && ./bin/hummingbot_quickstart.py 2>> ./logs/errors.log"
